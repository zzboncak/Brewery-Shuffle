...
function getStateBreweries(userInput, page=1) {
    let baseUrl = `https://api.openbrewerydb.org/breweries?by_state=`;
    let url = baseUrl + userInput + `&page=${page}&per_page=50`;
    //console.log(url);
    fetch(url).then(response => response.json()).then(responseJson => logBreweries(responseJson, userInput, page));
}
...
function logBreweries(responseJson, userInput, page) {
